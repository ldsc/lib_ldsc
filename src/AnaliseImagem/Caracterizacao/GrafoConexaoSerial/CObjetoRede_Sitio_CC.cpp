...
void CObjetoRede_Sitio_CC::DeletarConeccao (unsigned int link)
{
  ...
  // CObjetoRede_Sitio::DeletarConeccao(unsigned int link);
  // replicada abaixo
  vector < CObjetoRede * >::iterator it_coneccao = this->coneccao.begin ();
...
/** Marca e deleta os links para objetos invalidados (marcados para deleção).
  ...
  @return : void
*/
bool CObjetoRede_Sitio_CC::DeletarConeccoesInvalidadas (int deletado)
{
  long int cont = 0;
...
*/
// NOVO EM TESTE
int CObjetoRede_Sitio_CC::DeletarConeccoesRepetidas ()
{
