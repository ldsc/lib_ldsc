...
#include <Grafo/COGSitioEsqLR.h>
#endif

#ifndef COGSitioDirLR_h
#include <Grafo/COGSitioDirLR.h>
#endif
...
*/
void
CGra3Dby2D_M3::CalculoCondutancias
  (long double _viscosidade,
   long double _sizePixel, unsigned long int _fatorAmplificacao)
{
  // Variáveis auxiliares
  long double
    raio_hidraulico;
  ...
  long double
    sizePixelXfatorAmplificacao = _sizePixel * _fatorAmplificacao;
  // long double PI=3.141592653589;
  long double
    variavelAuxiliar = (CMath::PI)
  ...
  for (unsigned long int k = 0; k < objeto.size (); k++)
    {
      ...
      dH =
	raio_hidraulico + raio_hidraulico + raio_hidraulico + raio_hidraulico;
      ...
      COGSitio *
	ptrSitio = dynamic_cast < COGSitio * >(objeto[k]);
      ...
      for (unsigned int link = 0; link < ptrSitio->coneccao.size (); link++)
	{
	  ...
	  ptrSitioLR->condutancia[link] =
	    (variavelAuxiliar * dH * dH * dH * dH);
	}
    }
  return;
}
