...
//Construtor
CReconstrucaoEsferas::CReconstrucaoEsferas ( int _dimens, double _fat_sob, double _fat_borda ) : dimens(_dimens), fat_sob(_fat_sob), fat_borda(_fat_borda) {
	/*
	dimens = 100;
	fat_sob = 0.5;
	fat_borda = 0.9;
	*/
	srand(13);
	soma = 0.0;
	//i = flag = r = d = ptx = pty = ptz = d_cor = r_aux = porosidade = solido = perhist = dif = 0;
}

//=================== Executa a recontrução ==============================//
void CReconstrucaoEsferas::Go ( string nome_arq ) { 
	//================ Aloca a memoria ===========================================//
	...
	cout << "D_max: " << dmax << endl;
	
	//----------------------------------------------------------------------------//
	
	char str_famp[50];
	filein.getline( str_famp, 50, ':' );
	...
	cout << str_tmpix << " (1e-6m): " << tmpix << endl << endl;
	...
	for ( d = dmax_ef; d > 0; d-- ) {
		...
		r_aux = r + 1;
		for ( i = 0; i < hist[d]; i++ ) {
			cout << "\rDiametro = " << d << "  hist[d] = " << hist[d] << "  esf = " << i << "          			";
			...
			if ( (double)d <= fat_sob * (double)dmax_ef && flag_esf ) {
				flag = VerBorda ( inimeio, r_aux, ptx, pty, ptz, dimens );
				while ( flag == 0 ) {
					ptx = rand()%dimens;
					pty = rand()%dimens;
					ptz = rand()%dimens;
			...
			vol_sbr = Esfera (inimeio, r, r_aux, ptx, pty, ptz, dimens, fat_borda, &vol_esf );
	...
	double phi_ger = Calc_phi ( inimeio, dimens );
	...
	//char img_dat[50]; 
	//sprintf ( img_dat, "Meio_%s_%3d_Fs=%1.1f_FB=%1.2f.dat", nome_arq, dimens, fat_sob, fat_borda );
	stringstream img_dat;
	img_dat << "Meio_" << nome_arq << "_" << dimens << "_Fs=" << fat_sob << "_FB=" << fat_borda << ".dat";
	
	//char img_vtk[50]; 
	//sprintf ( img_vtk, "Meio_%s_%3d_Fs=%1.1f_FB=%1.2f.vtk", nome_arq, dimens, fat_sob, fat_borda );
	stringstream img_vtk;
	img_vtk << "Meio_" << nome_arq << "_" << dimens << "_Fs=" << fat_sob << "_FB=" << fat_borda << ".vtk";
	
	Grava_dat ( img_dat.str(), inimeio, porosidade, tmpix, dimens );
	Grava_vtk ( img_vtk.str(), inimeio, dimens );
} // Fim Go()...

//=================== Verificação da borda ===================================//

int CReconstrucaoEsferas::VerBorda ( unsigned char *inimeio, int raio, int ptx, int pty ,int ptz , int dimens ) {
	unsigned char *meio;
	int x, y, z;
	int xx, yy, zz;
	int flag = 0;
...
int CReconstrucaoEsferas::Esfera ( unsigned char *inimeio, int r, int r_aux, int ptx, int pty, int ptz, int dimens, double fat_borda , int *vol_ger ) {
	unsigned char *meio;
	int vol_sobr = 0;
	int x, y, z;
	int xx, yy, zz;
...
void CReconstrucaoEsferas::Grava_dat ( string nomeimg, unsigned char *inimeio, double phi, double tmpix, int dim ) {
	unsigned char *meio;
	ofstream fmeio ( nomeimg.c_str() );
	fmeio << tmpix << "e-6" << endl;
	//fmeio << dim/10 << endl;
	fmeio << dim << " " << dim << " " << dim << endl;
	int poros = 0;
	for ( int z = 0; z < dim; z++ ) {
		for ( int y = 0; y < dim; y++ ) {
			for ( int x = 0; x < dim; x++ ) {
				meio = inimeio + x + y*dim + z*dim*dim;
				if ( *meio ) {
					fmeio << 1 << " " ;
					poros = poros + 1;
				} else {
	...
	fmeio.close();
	double phi_ger = (double)poros/(double)( dim *dim *dim );
	cout << "\n\nPorosidade gerada = " << phi_ger << endl;
}

//=================== Grava os resultados .vtk ===============================//

void CReconstrucaoEsferas::Grava_vtk ( string nomeimg, unsigned char *inimeio, int dim ) {
	unsigned char *meio;
	...
	fmeio_out << "DIMENSIONS " << dim << " " << dim << " " << dim << endl;
	fmeio_out << "ASPECT_RATIO 1 1 1" << endl;
	fmeio_out << "ORIGIN 0 0 0" << endl;
	fmeio_out << "POINT_DATA " << dim*dim*dim << endl;
...
double CReconstrucaoEsferas::Calc_phi ( unsigned char *inimeio, int dim ) {
	unsigned char *meio;
	int poros = 0;
	for ( int z = 0; z < dim; z++ ) {
		for ( int y = 0; y < dim; y++ ) {
