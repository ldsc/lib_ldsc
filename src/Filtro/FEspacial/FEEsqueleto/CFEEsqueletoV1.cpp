...
TCMatriz2D< int > *
CFEEsqueletoV1::Go (TCMatriz2D< int > * &imagem, unsigned int /*_tamanhoMascara*/ )
{
  if (idf == NULL)
    {
      ...
      for (j = espessuraBorda; j < idf->NY () - espessuraBorda + 1; j++)	// Percorre direcao y
	{
	  ...
	  for (i = espessuraBorda; i < idf->NX () - espessuraBorda + 1; i++)	// Percorre direcao x
		...
		{		// se obedecer as condicoes abaixo, descasca o ponto
		  ...
		  if (idfdata[i][jm1] > mip1)
		    idfdata[i][j] = 0;
		  else if (idfdata[i][j_1] > mip1)
		    idfdata[i][j] = 0;
		  else if (idfdata[im1][jm1] > mip1)
		    idfdata[i][j] = 0;
		  else if (idfdata[im1][j] > mip1)
		    idfdata[i][j] = 0;
		  else if (idfdata[im1][j_1] > mip1)
		    idfdata[i][j] = 0;
		  else if (idfdata[i_1][jm1] > mip1)
		    idfdata[i][j] = 0;
		  else if (idfdata[i_1][j] > mip1)
		    idfdata[i][j] = 0;
		  else if (idfdata[i_1][j_1] > mip1)
		    idfdata[i][j] = 0;
