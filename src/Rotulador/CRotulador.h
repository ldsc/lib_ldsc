...
class CRotulador
{
  // --------------------------------------------------------------Atributos
 protected:
  /** @brief Representa a cor de fundo da imagem, normalmente=0*/
  /*unsigned */ int FUNDO; // Tirei o unsigned por causa dos warnings, verificar efeito

  /** @brief Vetor de conversao (identifica rotulos validos e invalidos)*/
  CVetor *vConversao;
  ...
  /** @brief flag ativo se a imagem ja foi rotulada*/
  bool rotulado;
  ...
  /** @brief Vetor com a area dos objetos (inicia com objeto 0)	*/
  CVetor *areaObjetos;

  // vector< int*>  areaObjetos;
  /** @brief Vetor com o perimetro dos objetos (inicia com objeto 0)*/
 ...
 public:
  ...
  /** @brief Determina a area dos objetos */
  virtual void CalculaAreaObjetos () = 0;

  /** @brief Determina o perimetro dos objetos */
  virtual void CalculaPerimetroObjetos () = 0;

  /** @brief Determina o raioHidraulico (area e perimetro) dos objetos */
  ...
  /** @brief O rotulo inicial é sempre  0*/
  int RotuloInicial () const
  {
    return 0;
  }

  /** @brief O ultimo rotulo é o numero de objetos -1*/
  int RotuloFinal () const
  {
    return (numeroObjetos - 1);
  }

  /** @brief retorna numero de objetos */
  int NumeroObjetos () const
  {
    return numeroObjetos;
  }

  /** @brief Retorna area do objeto*/
  int AreaObjetos (int k) const;

  /** @brief Retorna permetro do objeto*/
  int PerimetroObjetos (int k) const;

  /** @brief Retorna o raio hidraulico do objeto*/
  double RaioHidraulicoObjetos (int k) const;

  /** @brief Retorna o vetor area dos objetos*/
  CVetor *PtrAreaObjetos () const
  {
    return areaObjetos;
  }

  /** @brief Retorna o vetor perimetro dos objetos*/
  CVetor *PtrPerimetroObjetos () const
  {
    return perimetroObjetos;
  }

  /** @brief Seta o valor de FUNDO da imagem*/
  void SetFundo ( int fundo )
  {
    FUNDO = fundo;
  }

  /** @brief Pega o valor de FUNDO da imagem*/
  int GetFundo ( ) const
  {
    return FUNDO;
  }
