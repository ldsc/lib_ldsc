 ...
 * Gera a estrutura de sítios baseado na avaliação de cada plano da imagem tridimensional (planos bidimensionais).
 * Vai ter mais de uma herdeira.
 * Observe que da forma como esta, sempre usa sítios (CSitio e herdeiros) e nunca CObjetoRede_Ligacao ou CObjetoRede_Componente.
 *
 * Lista dos herdeiros:
 * -CGrafoConexaoSerial_M1	Modelo_1 : Para cada pixel uma ligação; Condutancia do pixel.
 *
 * -CGrafoConexaoSerial_M2	Modelo_2 : Para cada objeto uma ligação; Condutancia média entre os dois objetos.
 *
 * -CGrafoConexaoSerial_M3	Modelo_3 : Para cada objeto uma ligação; Condutancia calculada sobre a area da intersecção.
 *
 * -CGrafoConexaoSerial_M4	Modelo_4 : Para cada objeto uma ligação; Condutancia calculada sobre a área da intersecção.
 * Adicionalmente calcula as distâncias entre os objetos, para correção das condutâncias.
 *
 * -CGrafoConexaoSerial_M5	Modelo_5 : ??
 *
 * -CGrafoConexaoSerial_M6 : O mesmo que o M4, mas no M6 centro de massa é armazenado nos objetos; adiciona cálculo tortuosidade;
 *
 * @author	André Duarte Bueno
 * @see		Grafo
 * @ingroup  HCGrafo
