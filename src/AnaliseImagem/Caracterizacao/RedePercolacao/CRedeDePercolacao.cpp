				...
				for ( auto &lig : pmor.second.SConexao() ) { //percorre as ligações conectadas ao sítio
					for ( auto &con : ptrMatObjsRede->matrizObjetos[lig.first].SConexao() ) {//percorre os sítios conectados a ligação
						if ( con.first != pmor.first ) { //se o sítio for diferente do sítio atual
							...
							gSLS = CondutanciaSitioLigacaoSitio(is1->second,ptrMatObjsRede->matrizObjetos[lig.first],is2->second, tamLigacao, dimensaoPixel, fatorAmplificacao );
...
double CRedeDePercolacao::CondutanciaSitioLigacaoSitio (CObjetoRedeDePercolacao &objImgSitio1, CObjetoRedeDePercolacao &objImgLigacao,CObjetoRedeDePercolacao &objImgSitio2, double &comprimento, double &dimensaoPixel, double &fatorAmplificacao) {
	double gSitio1 = CondutanciaSitio(objImgSitio1, dimensaoPixel, fatorAmplificacao);
	double gSitio2 = CondutanciaSitio(objImgSitio2, dimensaoPixel, fatorAmplificacao);
	double gLigacao = CondutanciaLigacao(objImgLigacao,comprimento,dimensaoPixel,fatorAmplificacao);
	return 1.0/( (1.0/gSitio1) + (1.0/gLigacao) + (1.0/gSitio2) ); //inverso da resistência
}
