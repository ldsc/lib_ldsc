...
#include <MetNum/Funcao/CFPHermiteRec.h>

// Construtor
// Deve calcular os parametros que sao utilizados na funcao
CFPHermiteRec::CFPHermiteRec (double _porosidade, double _yc,
			      int _numeroPontosSerie)
{
  ...
  auxiliar1 = (_porosidade - 1) / sqrt (_porosidade * (1 - _porosidade));
...
double
CFPHermiteRec::Go (double x, double y)
{
  double resultado;
  ...
  if (x <= yc)			// 2.0    // 2.0
    resultado =
      (double) exp (-x * x / 2.0) 
      * Hermite (m, x) * auxiliar1 / sqrt (2.0 * M_PI * fatorial-> Go (m));

  else
    resultado =
      (double) exp (-x * x / 2.0) 
      * Hermite (m, x) * auxiliar2 / sqrt (2.0 * M_PI *  fatorial-> Go(m));
...
double
CFPHermiteRec::Hermite (int mm, double y)
{
  double a, b, c;
