...
class CRotuladorIRA3D : public TCRotulador3D<int>
{
	...
	public:
		...
		CRotuladorIRA3D (TCMatriz3D<int> * _pm) : TCRotulador3D<int> ( _pm, 1, 0), ra(0), rmin(0)  {
		}

		/// Recebe o nome da matriz de disco, abre arquivo de disco e seta matriz
		CRotuladorIRA3D (std::string fileName) : TCRotulador3D<int> ( fileName, 1, 0), ra(0), rmin(0) {
		}
		...
		// ----------------------------------------------------------------Metodos
		/** Rotula a IRA desconsiderando objetos menores que rmin.
		 ...
		 */
		bool Go (TCMatriz3D<int> * matriz, int _ra, unsigned int _rmin=0 );
