...
public:
  ...
  // int x0=0, y0=0, z0=0;									/// Coordenadas x0, y0 e z0 da imagem 3D
  // unsigned int fatorAmplificacao=1; /// Fator de amplificação utilizado na recontrução da imagem
  // double dimensaoPixel=1;               /// Resolução utilizada na obtenção da imagem pelo microscópio
  // unsigned int numeroPixeisBorda=0; /// Número de pixels que representam a borda da imagem (normalmente = 0).
  ...
  TCImagem3D (std::string nomeArquivo)
    : TCMatriz3D<T> (nomeArquivo),/* CImg(),*/ x0(0), y0(0), z0(0), fatorAmplificacao (1), dimensaoPixel (1) {
  ...
  /// Para atender código Thiago: pmatrix3D->getAmpFactor()
  /// Retorna o valor de fatorAmplificacao
  inline unsigned int getAmpFactor () const {
    return fatorAmplificacao;
  }
  ...
  /// Para atender código Thiago: pmatrix3D->getPixelSize()
