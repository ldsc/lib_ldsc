      ...
      for (int j = 0; j < ny; j++) {
         for (int i = 0; i < nx; i++) {
            ...
            if (data3D[i][j][k] > raioBolaInclusa) {
               pmdata3D[i][j][k] = INDICE; //indiceAtivo;	// seta ponto ativo
            } else {
               pmdata3D[i][j][k] = FUNDO; //indiceInativo;	// seta ponto inativo
            }
         }
      }
   }
      ...
      for (int ii = 0; ii < nx; ii++)	// pinta pontos na imagem
         for (int kk = 0; kk < nz; kk++)	// pinta pontos na imagem
            if (data3D[ii][jj][kk] >= mi)
               pmdata3D[ii][jj][kk] = INDICE; //indiceAtivo;	// pm->data3D[ii][jj][kk]=1;
            else
               pmdata3D[ii][jj][kk] = FUNDO; //indiceInativo;	// pm->data3D[ii][jj][kk]=0;
                     ...
                     for (xx = 0; xx <= raio; xx++) {
                        ...
                        if (maskd->data3D[rmx][rmy][rmz] != 0) {
                           pmdata3D[posxe][posyn][poszf] =
                                 ...
                                 pmdata3D[posxd][posys][poszb] = INDICE; //indiceAtivo;
                        }
                     }
                  }
               }
...
*/
void CFEMMIDF3D::InverterSeNecessario(){
   if (FUNDO != 0){ //inverte a imagem
      for ( int z = 0; z < ny; z++ ) {
         for ( int y = 0; y < ny; y++ ) {
            for ( int x = 0; x < nx; x++ ) {
               if ( data3D[x][y][z] == INDICE) {
                  data3D[x][y][z] = FUNDO;
               } else {
                  data3D[x][y][z] = INDICE;
               }
