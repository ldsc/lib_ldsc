...
// ostream & operator<< ( ostream & os, const CSMDiagonalDominante & s )
// {
...
// istream & operator>> ( istream & is, CSMDiagonalDominante & s )
// {
 ...
 * e atualizar as variáveis x nos objeto usando a função Go() do objeto.
 *
 ...
 */
double CSMDiagonalDominante::Go( vector < CSMParametroSolver * > * objeto )
{
   // Seta o atributo obj como sendo o vetor recebido. Necessário pois usa obj em AtualizaX.
   obj = objeto;

   if( vx == nullptr or dimensaoVetor < obj->size() ) {
         // Se menor
         delete [] vx;							// Deleta vx
         vx = nullptr;							// Zera
...
cerr << " depois de chamar AtualizaX -> vxk0 = " << vxk0 ;
               ...
               if( ( vx[k] - vxk0 ) != 0.0 ) {	// os objetos da borda tem vxk0 == vx[k] logo o erro = 0/0
                     ...
                     if( erro < erroPontual )
                        erro = erroPontual;			// erro assume o maior valor de todos os erros pontuais
                  }
            }
