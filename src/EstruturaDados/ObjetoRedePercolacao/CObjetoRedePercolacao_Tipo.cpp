/**
  ...
  @file:             CObjetoRedePercolacao_Tipo.cpp
...
// -----------------------------------------------------------------------
#include <EstruturaDados/ObjetoRedePercolacao/CObjetoRedePercolacao_Tipo.h>
using namespace std;
...
/** Marca e deleta os links para objetos invalidados (marcados para deleção).
  ...
  @return : void
*/
bool CObjetoRedePercolacao_Tipo::DeletarConexoesInvalidadas( unsigned int deletado )
{
...
*/
unsigned int  CObjetoRedePercolacao_Tipo::DeletarConexoesRepetidas_e_SomarCondutanciasParalelo()
{
...
int CObjetoRedePercolacao_Tipo::DeletarConeccoesRepetidas_V2 ()
{
  // cria um vetor de pair.
  vector < pair<CObjetoRedePercolacao_Tipo *, long double> > par_conexao_condutancia(conexao.size());
  // preenche o par.
  for (unsigned int i = 0 ;  i< conexao.size(); i++)
  ...
  sort(begin(par_conexao_condutancia), end(par_conexao_condutancia),
       []( pair<CObjetoRedePercolacao_Tipo *, long double> par ) { par.first }  ); // corrigir comparação
...
@return : void
*/
/*inline*/ ostream & CObjetoRedePercolacao_Tipo::Write( ostream & out ) const
{
   out.setf( ios::right );
...
/** Escreve os atributos do objeto em disco.
    ...
    Sobrecarga operador entrada streams (antiga CObjetoRedePercolacao_TipoEsqueleto).
    @author :	André Duarte Bueno
     
    @param  : 	ostream& e CObjetoRedePercolacao_Tipo&
    @return :		ostream&
*/
ostream & operator<< ( ostream & out, CObjetoRedePercolacao_Tipo & s )
{
   s.Write( out );
   return out;
}
...
/** Sobrecarga operador entrada streams
    ...
    @param  : istream& is, CObjetoRedePercolacao_Tipo& s
    @return : istream&
*/

/*istream& operator>> (istream& is, CObjetoRedePercolacao_Tipo& s)
  COPIAR DE CObjetoRedePercolacao
  }   */
...
/** A função Go calcula o novo valor de x, considerando o relacionamento
    ...
    @return : long double
*/
long double CObjetoRedePercolacao_Tipo::Go( long double /*d */ )
{
...
*/
long double CObjetoRedePercolacao_Tipo::Fluxo() const
{
