...
#ifndef TFEEsqueletoV43D_cpp
#define TFEEsqueletoV43D_cpp
/*
----------------------------------------------------------------------------
PROJETO:		Anaimp.
...
#include "TFiltro\TFEspacial3D\TFEEsqueleto3D\TFEEsqueletoV43D.h"
...
TMatriz3D *
TFEEsqueletoV43D::Go (TMatriz3D * &imagem, unsigned int _tamanhoMascara)
{
  if (idf == NULL)
    {
      idf = new TFMMIDFd3453D (imagem);	//Cria a idf, e testa alocacao
      TOperacao::TestaAlocacao (idf,
				"Erro alocacao idf, funcao Go, objeto TFEEsqueletoV43D");
    }
      ...
      for (k = espessuraBorda; k < idf->GetNZ () - espessuraBorda + 1; k++)	//Percorre direcao z
	{
	  ...
	  for (j = espessuraBorda; j < idf->GetNY () - espessuraBorda + 1;
		   ...
		   i++)
		{
		  im1 = i + 1;
		  i_1 = i - 1;
		  if (idf->data3D[i][j][k] == mi)	//agora tem menos pontos 3
		    {
