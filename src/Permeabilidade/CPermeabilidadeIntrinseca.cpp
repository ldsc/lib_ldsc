...
bool CPermeabilidadeIntrinseca::CriarObjetos (CMatriz3D * matriz3D, unsigned int fatorAmplificacao, 
	double sizePixel, unsigned int numeroPixelsBorda, long double fatorRelaxacao) 
{
	...
	if ( ! grafo  )
		 { DestruirObjetos();	return false; } 	// se não criou o objeto, destroi os objetos já criados e retorna false.
...
long double CPermeabilidadeIntrinseca::CalcularPermeabilidade( CMatriz3D * matriz3D )
{
	...
	cout << "fluido->Compressibilidade() = " 	<< fluido->Compressibilidade() << endl;
	cout << "fluido->Densidade() = " 			<< fluido->Densidade() << endl;
	cout << "fluido->Molhabilidade() = " 		<< fluido->Molhabilidade() << endl << endl;
	
	cout << "solver->LimiteIteracoes() = " 		<< solver->LimiteIteracoes()<< endl;		
	cout << "solver->LimiteErro() = " 			<< solver->LimiteErro() << endl << endl;
	
	cout << "perm->GetfatorAmplificacao() = " 	<< perm->GetfatorAmplificacao()<< endl;
	cout << "perm->GetsizePixel() = " 			<< perm->GetsizePixel()<< endl;
	cout << "perm->GetnumeroPixelsBorda() = " 	<< perm->GetnumeroPixelsBorda() << endl << endl;
...
void CPermeabilidadeIntrinseca::SetarPropriedadesSolver( long double _limiteErro, unsigned long int _limiteIteracoes )//, long double _fatorRelaxacao )
{
