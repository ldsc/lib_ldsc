...
TCMatriz2D< int > * CFEBorda::Go (TCMatriz2D< int > * &imagem, unsigned int espessura) {
	int nx = imagem->NX ();
	int ny = imagem->NY ();
	int nx_1 = nx - 1;
	int ny_1 = ny - 1;
	...
	for (desloc = 0; desloc < espessura; desloc++) {
		for (int x = desloc; x < nx - desloc; x++) {
			...
			imagem->data2D[x][desloc] = imagem->data2D[x][ny_1 - desloc] = valorBorda;
		}
		for (int y = desloc + 1; y < ny - desloc - 1; y++) {
			// Funciona: imagem->data2D[desloc][y]     =     imagem->data2D[nx_1-desloc][y]=valorBorda;
			imagem->data2D[desloc][y] = imagem->data2D[nx_1 - desloc][y] = valorBorda;
		}
	}
	return imagem;
}
