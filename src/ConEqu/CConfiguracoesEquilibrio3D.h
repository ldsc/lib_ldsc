...
public:
  ...
  friend ostream & operator<< (ostream & os, CConfiguracoesEquilibrio3D & configuracoesEquilibrio3D) { return os; } 
  
  /// Sobrecarga do operador >>
  // IMPLEMENTAR - Deve obter da entrada os atributos do objeto
  friend istream & operator>> (istream & is, CConfiguracoesEquilibrio3D & configuracoesEquilibrio3D) { return is; } 
 ...
 // bool GetImagem(CMatriz3D * imagem, int indice_regiao) const;
  ...
  int GetIndiceRegiaoA() { return A; }
  
  /// Retorna o índice da região B	
  int GetIndiceRegiaoB() { return B; }	
  
  /// Retorna o índice da região Yi
  int GetIndiceRegiaoY() { return Yi;}	
  ...
  void TipoIDF (const int &_tipoIDF) 		{ tipoIDF = _tipoIDF; } 
  
  /// Método que retorna o tipo de idf
  int TipoIDF () const				{ return tipoIDF ; } 
 	
  ///  Somente para teste, retorna this
  const CConfiguracoesEquilibrio3D* This ()const{ return this; } 
 
  /// Get salvarResultadosParciaisDisco
  bool SalvarResultadosParciaisDisco() 	const	{ return salvarResultadosParciaisDisco; };
 
  /// Set salvarResultadosParciaisDisco
  bool SalvarResultadosParciaisDisco(bool s) 	{ salvarResultadosParciaisDisco = s; };
  
  /// Get salvarResultadosFinaisDisco
  bool SalvarResultadosFinaisDisco() 	const	{ return salvarResultadosFinaisDisco; };
 
  /// Set salvarResultadosParciaisDisco
  bool SalvarResultadosFinaisDisco(bool s) 	{ salvarResultadosFinaisDisco = s; };
		  
		  
  /// Get Saturação da fase A
  double SaturacaoA() const			{ return saturacaoA; }
 
  /// Get Saturação da fase B
  double SaturacaoB()  const 			{ return saturacaoB; }
