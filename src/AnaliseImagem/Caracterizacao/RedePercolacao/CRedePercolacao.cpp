#include "CRedePercolacao.h"
#include <map>

// Analizar se é melhor calcular a porosidade assim ou se devo calcular a área da esfera e do cilindro.
//int CRedePercolacao::numPixeisBola[] = {
std::vector<unsigned int> CRedePercolacao::numPixeisBola =
...
std::vector<unsigned int> CRedePercolacao::numPixeisCirculo
...
std::vector<unsigned short int> CRedePercolacao::perimetroCirculo
...
CRedePercolacao::CRedePercolacao(unsigned short int _nx, unsigned short int _ny, unsigned short int _nz )
	:CRedeContorno("nomeArquivoIndefinido")
{
...
// Destrutor
CRedePercolacao::~CRedePercolacao(){
	if (dtpg.first)
		delete dtpg.first;
	if (dtpg.second)
		delete dtpg.second;
...
// g = (r^3) / (3*viscosidade) ->
double CRedePercolacao::CondutanciaSitio (CObjetoRedeDePercolacao &objetoImagem, double dimensaoPixel, double fatorAmplificacao) {
...
double CRedePercolacao::CondutanciaLigacao (CObjetoRedeDePercolacao &objetoImagem, double &_comprimento, double dimensaoPixel, double fatorAmplificacao) {
	// Variáveis auxiliares
	double viscosidade = 1.0;
...
double CRedePercolacao::CondutanciaSitioLigacao (CObjetoRedeDePercolacao &objImgSitio, CObjetoRedeDePercolacao &objImgLigacao, double &comprimento, double dimensaoPixel, double fatorAmplificacao) {
...
bool CRedePercolacao::Go( TCImagem3D<bool> *&_pm, int _raioMaximo, int _raioDilatacao, int _fatorReducao, int _incrementoRaio, EModelo _modelo, int _indice, int _fundo, CDistribuicao3D::Metrica3D _metrica ){
	...
	if (dtpg.first == nullptr || dtpg.second == nullptr) {
		std::cerr << "Não foi calcular as distribuições de tamanho de poros e gargantas em CRedePercolacao::Go" << std::endl;
		return false;
	}
...
// Executa o cálculo das distribuições e cria a rede de percolação.
bool CRedePercolacao::Go( TCImagem3D<int> *&_pm, CDistribuicao3D::Metrica3D _metrica ) {
...
// Executa o cálculo das distribuições e cria a rede de percolação.
bool CRedePercolacao::ExecutadaPorGo( ) {
...
bool CRedePercolacao::SalvarListaObjetos(std::string nomeArquivo) const {
	return false;/// ptrMatObjsRede->SalvarListaObjetos(nomeArquivo, nx, ny, nz);
}

// Grava em disco, no formato do Grafo, com o nome informado, os objetos identificados.
bool CRedePercolacao::SalvarListaObjetosGrafo(std::string nomeArquivo) const {
	return false;/// ptrMatObjsRede->SalvarListaObjetosGrafo(nomeArquivo);
}

// Write: Salva Rede em disco
void CRedePercolacao::Write ( std::ostream& os ) const {
	/** os.setf ( ios::left );
