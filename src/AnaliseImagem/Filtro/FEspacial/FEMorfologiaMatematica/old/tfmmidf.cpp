...
Descricao:	 Implementa as fun��es da classe CFMMIDF.
*/
...
// por default, n�o recalcula a idf apos dilatacao ou fechamento
bool
...
2-Copia valores da matriz passada para a matriz idf, usando data2D[i][j]=pm->data2D[i][j]
	...
	Esta fun��o foi inclu�da porque o usuário programador pode chamar Go(pm)
...
CFMMIDF::Abertura (CMatriz2D * &matriz, unsigned int _RaioBola)	// 
{
  ...
  // vari�veis auxiliares para otimizacao
  // Otimizacao Mascara
  ...
  // Vari�veis para SIMETRIA Bola   // otimizacao
...
// PINTA A BOLA NAO OTIMIZADO: NAO CONSIDERA SIMETRIA
             ...
             imx=i+xx; rmx=raio+xx;			  // variaveis otimizacao
               ...
               if(maskd->data2D[rmx][raio+yy]!=0)// se o ponto da bola � ativo
...
// PINTA A BOLA OTIMIZADO: CONSIDERA SIMETRIA
	    for (xx = 0; xx <= raio; xx++)	// Usa simetria para pintura da bola
	      {			// s� percorre meia bola
		posxe = i - xx;	// por ser sim�trica
		posxd = i + xx;
		rmx = raio + xx;
		for (yy = 0; yy <= raio; yy++)
		  {
		    ...
		    posyn = j + yy;
		    if (maskdata2D[rmx][raio + yy] != 0)	// se o ponto da bola � ativo
...
// NAO OTIMIZADO
             ...
             imx=i+x; rmx=raio+x;			  // variaveis otimizacao
               ...
               if(maskd->data2D[rmx][raio+y]!=0)// se o ponto da bola � ativo
...
CFMMIDF::Esqueleto (CMatriz2D * &matriz, unsigned int /*_RaioBola*/ )
{
  ...
  for (int j = 0; j < ny; j++)	// percorre toda a idf e
	...
	raioMaximo = data2D[i][j];	// � na verdade o maior valor da matriz
  raioMaximo /= mi;		// agora � o raioMaximo  0->raioMaximo

  // ----------------------------------------
  //  Percorre a imagem de mi at� raio maximo
  // ----------------------------------------
	  ...
	  for (int i = 2; i < nx - 1; i++)
	      ...
	      if (data2D[i][j] == raioBolaInclusa)	// pesquisa
		{
		  if (data2D[i][j + 1] > raioBolaInclusa)
		    ...
		    continue;	// o raio da bola inclusa, eu n�o sou
		    ...
		    continue;	// maior que o raioBolaInclusa, ent�o
		  else if (data2D[i - 1][j - 1] > raioBolaInclusa)
...
/*
============================================================================
Fun��o    DilatacaoNosContornos
============================================================================
Como a mascara nao pode ser encaixada nos pontos de contorno, estes s�o
