...
#include <Geometria/Bola/BCDiscreta/CBCEuclidiana.h>	// Classe base
...
//  Se me lembro bem, a multiplicação só é feita porque usa int e não float
  CBCEuclidiana::CBCEuclidiana (unsigned int tm):
CBCdij (tm, 100, CMath::MSQRT2 * 100, 100)
{
      ...
      for (yy = 0; yy <= raio; yy++)	// 
	{
	  ...
	  if (dist <= raio)	// se pertence a bola
	    data2D[posxe][posyn] = data2D[posxe][posys] =
	      data2D[posxd][posyn] = data2D[posxd][posys] = dist * 100;
	  else
	    data2D[posxe][posyn] = data2D[posxe][posys] =
