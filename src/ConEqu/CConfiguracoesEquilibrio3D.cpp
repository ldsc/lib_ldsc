...
#include <string>
#include <sstream>
#include <cstdio>			//  sprintf(msg,"mmmmm %d",int);
...
*/
void
CConfiguracoesEquilibrio3D::CriaCamara (CMatriz3D * &imagem)
{
  if (camara) {
  	delete camara;
  	camara = NULL;
...
void
CConfiguracoesEquilibrio3D::CriaIDF ()
{
  if (idf)
...
void
CConfiguracoesEquilibrio3D::CriaRotulador ()
{
  if (rotulador)
...
void
CConfiguracoesEquilibrio3D::CalculaAbertura (int &raio)
{
  CMatriz3D* ptr_camara = static_cast<CMatriz3D*>(camara);
...
void
CConfiguracoesEquilibrio3D::RestabeleceIDFPositiva ()
{
