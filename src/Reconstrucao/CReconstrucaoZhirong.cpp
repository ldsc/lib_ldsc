...
CReconstrucaoZhirong::CReconstrucaoZhirong (std::string nomeArquivo, int fatorAmp, int npcz, double sp, int nx, int ny, int nz) : CReconstrucao ()
{
  ...
  ostringstream os;
  os << "n" << fatorAmplificacaoNumeroPontos << "-np" << numeroPontosCz << "-N" << NX << "-";
  os << nomeArquivoCz.substr( nomeArquivoCz.find_last_of("/\\") + 1, nomeArquivoCz.find_last_of(".") - nomeArquivoCz.find_last_of("/\\") - 1 ) << ".dbm";
  fileName = os.str();
...
// ----------------------------------------
void CReconstrucaoZhirong::SalvarImagemEmDisco (string arqimg)
{
	if ( fileName == arqimg ) {
...
{
  time_t ti, tf;
  ofstream fout ("tela.txt");

  fout << "\nLendo arquivo com valores de Cz do disco ...";
  ti = time (NULL);
  LeituraArquivoCz ();
  tf = time (NULL);
  cout << "\nTempo decorrido processo (LeituraArquivoCz)" <<
    (difftime (tf, ti));
  fout << "...Cz ok.";
  ...
  fout << " ...objeto CENormal criado.";
  normal->limiteErro = 1e-10;
  yc = normal->GoInv (porosidade);	// retorna yc da eq 3.15
  tf = time (NULL);
  cout << "\nTempo decorrido processo (normal->GoInv(porosidade);)" <<
    (difftime (tf, ti));
  ...
  fout << " ...objeto TFFT_Properties criado.";
  ...
  tf = time (NULL);
  cout << "\nTempo decorrido processo (objetoFourier->Go_Inverse)" <<
    (difftime (tf, ti));
  fout << "...Y ok.";		/* 3D inverse FFT is now completed */
  ...
  cout <<
    "\nTempo decorrido processo (estatistica->Covariancia(Re_data,NX,NY,NZ);)"
    << (difftime (tf, ti));

  fout << " media=" << estatistica->Media();
  
  fout << " covariancia=" << estatistica->Covariancia();
  
  if (fabs (estatistica->Media() ) > 0.1 || fabs ( estatistica->Covariancia() ) > 1.2)
    {
      fout << "\nCUIDADO!\tValores de média e covariância ruins";
      fout <<
	"\nFavor checar a função correlação de entrada e os parâmetros de entrada.\n";
    }
  ...
  fout << "\nFim da função CReconstrucaoZhirong::Go.";
