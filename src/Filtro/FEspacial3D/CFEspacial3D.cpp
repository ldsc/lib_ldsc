#ifndef CFEspacial3D_cpp
#define CFEspacial3D_cpp
...
Nome deste arquivo:	CFEspacial3D.cpp
Nome da classe:      CFEspacial3D
...
*/
void
CFEspacial3D::CriaMascara (unsigned int _tamanhoMascara)
{
  if (mask)
...
CFEspacial3D::Go (TCMatriz3D<int> * &matriz, unsigned int _tamanhoMascara)
{
  ...
  for (i = (raioMascaraX); i < (pm->NX () - raioMascaraX); i++)
    for (j = (raioMascaraY); j < (pm->NY () - raioMascaraY); j++)
      for (k = (raioMascaraZ); k < (pm->NZ () - raioMascaraZ); k++)
	{
	  ...
	  for (ii = 0; ii < mask->NX (); ii++)	// percorre a mascara
	    for (jj = 0; jj < mask->NY (); jj++)
	      for (kk = 0; kk < mask->NZ (); kk++)
		{		// realiza convolução da mascara com a imagem

		  soma +=
		    mask->data3D[ii][jj][kk] 
		    * rImg->data3D[i + ii - raioMascaraX][j + jj - raioMascaraY][k + kk - raioMascaraZ];
		}
	  ...
	  if (soma > 0)
	    pm->data3D[i][j][k] = soma / mask->Peso ();
	  else
	    pm->data3D[i][j][k] = 0;
	}
