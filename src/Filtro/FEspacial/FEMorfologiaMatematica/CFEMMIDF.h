 ...
 * Herdeira de CFEMorfologiaMatematica e TMatriz2D< int >.
 */
class CFEMMIDF : public CFEMorfologiaMatematica, public TMatriz2D< int > {
   // Atributos
protected:
...
protected:
   ...
   inline void VerificaImagem ( TMatriz2D< int > * &matriz );

   /// Verifica se a imagem passada é a mesma, se diferente recalcula Go.
   // antiga InicializaIDF
   void ExecutadaPorGo ( TMatriz2D< int > * &matriz );
...
public:
   ...
   CFEMMIDF ( TMatriz2D< int > * &matriz, unsigned int _tamanhoMascara = 3, unsigned int _raioMax = 32000, int _indice=1, int _fundo=0 );
   ...
   void CorrigeAbertura ( TMatriz2D< int > * &matriz, int &regiao );
};
#endif
