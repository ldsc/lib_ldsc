...
TCMatriz2D< int > * CFEEsqueletoV4::Go (TCMatriz2D< int > * &imagem, unsigned int /*_tamanhoMascara*/ ) {
	if (idf == NULL) {
		...
		for (j = espessuraBorda; j < idf->NY () - espessuraBorda + 1; j++)
		{
			...
			for (i = espessuraBorda; i < idf->NX () - espessuraBorda + 1; i++)
			{
				...
				if (idf->data2D[i][j] == mi)	// agora tem menos pontos 3
				{
					...
					somaVizinhos = 0;
					if (idf->data2D[i][jm1] > 3)
						somaVizinhos += idf->data2D[i][jm1];
					if (idf->data2D[i][j_1] > 3)
						somaVizinhos += idf->data2D[i][j_1];
					if (idf->data2D[im1][j] > 3)
						somaVizinhos += idf->data2D[im1][j];
					if (idf->data2D[i_1][j] > 3)
						somaVizinhos += idf->data2D[i_1][j];
					// diagonal
					if (idf->data2D[im1][jm1] > 3)
						somaVizinhos += idf->data2D[im1][jm1];
					if (idf->data2D[im1][j_1] > 3)
						somaVizinhos += idf->data2D[im1][j_1];
					if (idf->data2D[i_1][jm1] > 3)
						somaVizinhos += idf->data2D[i_1][jm1];
					if (idf->data2D[i_1][j_1] > 3)
						somaVizinhos += idf->data2D[i_1][j_1];

					if (somaVizinhos >= 10)
						idf->data2D[i][j] = 0;	// descasca
				}
			}
		}
