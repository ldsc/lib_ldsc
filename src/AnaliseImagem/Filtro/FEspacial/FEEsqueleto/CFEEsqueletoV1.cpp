		...
		for (j = espessuraBorda; j < idf->NY () - espessuraBorda + 1; j++) {	// Percorre direcao y
			...
			for (i = espessuraBorda; i < idf->NX () - espessuraBorda + 1; i++) {	// Percorre direcao x
				...
				if (idf->data2D[i][j] == mi) {	// faz a pesquisa no ponto  // se igual a mi // mi=3 para d34 e d345
					...
					if (idf->data2D[i][jm1] > mip1)
						idf->data2D[i][j] = 0;
					else if (idf->data2D[i][j_1] > mip1)
						idf->data2D[i][j] = 0;
					else if (idf->data2D[im1][jm1] > mip1)
						idf->data2D[i][j] = 0;
					else if (idf->data2D[im1][j] > mip1)
						idf->data2D[i][j] = 0;
					else if (idf->data2D[im1][j_1] > mip1)
						idf->data2D[i][j] = 0;
					else if (idf->data2D[i_1][jm1] > mip1)
						idf->data2D[i][j] = 0;
					else if (idf->data2D[i_1][j] > mip1)
						idf->data2D[i][j] = 0;
					else if (idf->data2D[i_1][j_1] > mip1)
						idf->data2D[i][j] = 0;
